...
utility <- function(threshold, scale.points, coverage.parameter, densityf, cumulativef) {
  ...
  U <- ES + coverage.parameter * cumulativef(threshold)
  return(U)
}


probability.threshold <- function(threshold, scale.points, lambda, coverage.parameter, densityf, cumulativef) {
  P <- exp((lambda * utility(threshold, scale.points, coverage.parameter, densityf, cumulativef))) / 
...
use.adjective <- function(degree, scale.points, lambda, coverage.parameter, densityf, cumulativef) {
  P.thresholds.D <- P.thresholds[scale.points <= degree]
  sigma <- sum(P.thresholds.D)
  return(sigma)
}
...
lambda <- 50
coverage.parameter <- 0
densityf <- function(x) {dnorm(x, 180, 10)}
cumulativef <- function(x) {pnorm(x, 180, 10)}
P.thresholds <- sapply(scale.points, function(x) {probability.threshold(x, scale.points, lambda, coverage.parameter, densityf, cumulativef)})

probPerScalePoint <- data.frame("Probability" = P.thresholds, "Height" = scale.points)

g2 <- ggplot(probPerScalePoint, aes(x=Height, y=Probability)) 
g2 <- g2 + geom_area(fill="green", alpha=.4) 
g2 <- g2 + xlab("Height (in cm)")

sigma.thresholds <- sapply(scale.points, function(x) {use.adjective(x, scale.points, lambda, coverage.parameter, densityf, cumulativef)})

probPerDegreePoint <- data.frame("Probability" = sigma.thresholds, "Height" = scale.points)

g3 <- ggplot(probPerDegreePoint, aes(x=Height, y=Probability)) 
g3 <- g3 + geom_area(fill="green", alpha=.4) 
g3 <- g3 + xlab("Height (in cm)")

scale.points[which(P.thresholds == max(P.thresholds))]
