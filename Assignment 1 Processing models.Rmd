...
C.
The plot shows the strategies used for the model predictionsthat are comparable in their performance to participats in the steer-focus condition. We can see that participants do not solely seem to interleave at the natural breakpoint. For some strategies, they do interleave at the natural breakpoint, but there are also strategies where they already interleave before the natural breakpoint, and a few where they interleave at a later point than de natural breakpoint. Furthermore, in the simulation interleaving only occurs one or two times. 
Because a great part of the model predictions that fall within human performance do not interleave only at the natural breakpoint, it does not seem plausible that human solely interleaved at the natural breakpoint. 
...
We only changed the value of the gaussDeviateSD parameter to compare the use of different drift versions on the model fit. 
