...
runAllComplexStrategies <- function(nrSimulations,phoneNumber) {
	...
	ggplot(data = agrResultsMeanDrift) +
	  geom_point(aes(TrialTime/1000, abs(dev), color = "Simulations")) +
	  xlab("Dial time (s)") +
	  ylab("Average Lateral Deviation (m)") +
	  geom_point(aes(x= (dialingTime[1] / 1000), y=laneDev[1]), shape = 1, color = "Dial focus trials") +
	  geom_point(aes(x= (dialingTime[4] / 1000), y=laneDev[4]), shape = 2, color = "Steer focus trials") +
	  geom_errorbar(aes(x = (dialingTime[1] / 1000), ymin = laneDev[1] - laneDev[3], ymax = laneDev[1] + laneDev[3]), size = 1, width = 0.2, position = position_dodge(0.05), color = "Dial focus trials") +
	  geom_errorbar(aes(x = (dialingTime[4] / 1000), ymin = laneDev[4] - laneDev[6], ymax = laneDev[4] + laneDev[6]), size = 1, width = 0.2, position = position_dodge(0.05), color = "Steer focus trials") +
	  geom_errorbarh(aes(y = laneDev[1], xmin = (dialingTime[1] / 1000) - (dialingTime[3] / 1000), xmax = (dialingTime[1] / 1000) + (dialingTime[3] / 1000)), size = 1, width = 0.2, position = position_dodge(0.05), height = 0.05, "Dial focus trials") +
	  geom_errorbarh(aes(y = laneDev[4], xmin = (dialingTime[4] / 1000) - (dialingTime[6] / 1000), xmax = (dialingTime[4] / 1000) + (dialingTime[6] / 1000)), size = 1, width = 0.2, position = position_dodge(0.05), height = 0.05, color = "Steer focus trials") +
...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
library(dplyr)
agrResultsMeanDrift$TrialTime <-agrResultsMeanDrift$TrialTime / 1000

stratsDial <- agrResultsMeanDrift %>%
  filter(TrialTime > (dialingTime[1] - dialingTime[3]) & TrialTime < (dialingTime[1] + dialingTime[3])) %>%
           filter(abs(dev) > (laneDev[1] - laneDev[3]) & abs(dev) < (laneDev[1] + laneDev[3]))

stratsSteer <- agrResultsMeanDrift %>%
  filter(TrialTime > (dialingTime[4] - dialingTime[6]) & TrialTime < (dialingTime[4] + dialingTime[6])) %>%
           filter(abs(dev) > (laneDev[4] - laneDev[6]) & abs(dev) < (laneDev[4] + laneDev[6])) 
