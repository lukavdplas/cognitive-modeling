...
runAllComplexStrategies <- function(nrSimulations,phoneNumber) {
	...
	for (i in 1:10)  #nrow(all_strategies))
	{
	...
	ggplot(agrResultsMeanDrift, aes(TrialTime/1000, abs(dev))) +
	  geom_point() +
	  xlab("Dial time (s)") +
	  ylab("Average Lateral Deviation (m)") +
	  geom_point(aes(x=dialingTime[1], y=laneDev[1]), shape = 1, color = "blue") +
	  geom_point(aes(x=dialingTime[4], y=laneDev[4]), shape = 2, color = "blue")
...
library(ggplot2)
runAllComplexStrategies(nrSimulations = 1)
